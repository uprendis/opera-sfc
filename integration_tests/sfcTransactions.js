...
class TransactionHandler {
    ...
    getStakersNum(from) {
        return new Promise((resolve) => {
            this.sfc.methods.stakersNum().call({ from }, (error, result) => {
                if (!error) {
                    resolve(result);
                } else {
    ...
    getStaker(from, stkrId) {
        return new Promise((resolve) => {
            ...
            this.sfc.methods.stakers(stkrId).call({ from }, (error, result) => {
                if (!error) {
                    resolve(result);
                } else {
                    console.log('getStakersNum err', error);
    ...
    getDelegation(from, delegatorAddr) {
        return new Promise((resolve) => {
            // console.log("this.sfc.methods.stakers", this.sfc.methods.stakers);
            this.sfc.methods.delegations(delegatorAddr).call({ from }, (error, result) => {
                if (!error) {
                    resolve(result);
                } else {
                    console.log('getStakersNum err', error);
    ...
    stakeLockPeriodTime(from) {
        return new Promise((resolve) => {
            // console.log("this.sfc.methods.stakers", this.sfc.methods.stakers);
            this.sfc.methods.stakeLockPeriodTime().call({ from }, (error, result) => {
                if (!error) {
                    resolve(result);
                } else {
                    console.log('getStakersNum err', error);
    ...
    stakeLockPeriodEpochs(from) {
        return new Promise((resolve) => {
            // console.log("this.sfc.methods.stakers", this.sfc.methods.stakers);
            this.sfc.methods.stakeLockPeriodEpochs().call({ from }, (error, result) => {
                if (!error) {
                    resolve(result);
                } else {
                    console.log('getStakersNum err', error);
    ...
    currentSealedEpoch(from) {
        return new Promise((resolve) => {
            // console.log("this.sfc.methods.stakers", this.sfc.methods.stakers);
            this.sfc.methods.currentSealedEpoch().call({ from }, (error, result) => {
                if (!error) {
                    resolve(result);
                } else {
    ...
    getStakerId(from, stakerAddr) {
        return new Promise((resolve) => {
            this.sfc.methods.getStakerID(stakerAddr).call({ from }, (error, result) => {
                if (!error) {
                    resolve(result);
                } else {
    ...
    sfcAddressToStakerID(from) {
        return new Promise((resolve) => {
            ...
            this.sfc.methods.getStakerID(from).call({ from }, (error, result) => {
                if (!error) {
                    resolve(result);
                } else {
    ...
    sfcAddressToStakerIDNew(from) {
        return new Promise((resolve) => {
            console.log('from', from);
            this.sfc.methods._sfcAddressToStakerID(from).call({ from }, (error, result) => {
                if (!error) {
                    resolve(result);
                } else {
                    console.log('sfcAddressToStakerID err', error);
    ...
    async updateContract(from, data) {
        ...
        const estimatedGas = await this.estimateGas(rawTx);
    ...
    async newSignedTx({
        ...
        const tx = await this.web3.eth.accounts.signTransaction(rawTx, accountFrom.privateKey);
        return this.web3.eth.sendSignedTransaction(tx.rawTransaction, (err, hash) => {
            if (err) { throw (err); }
    ...
    async deployContract(abi, code) {
        const { web3 } = this;
        const newSfc = new web3.eth.Contract(abi);
        let coinBase;
        ...
        const contract = await newSfc.new({ from: web3.eth.coinbase, gas: 1000000, data: code });
