...
const host = program.host ? program.host : defaultHost;
const port = program.port ? program.port : defaultPort;
const endpoint = `http://${host}:${port}`;
const contractAddress = program.contractAddress ? program.contractAddress : defaultContractAddress;

let payer;
if (program.payer) {
    if (program.keyObject && program.password) {
        const keyObject = JSON.parse(program.keyObject);
        payer = {
            keyObject,
