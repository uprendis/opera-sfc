...
class lachesis {
    ...
    async findLastTransaction(maxBlocksToInspect, newContractAddress) {
        ...
        const latestBlockNumber = new BN(this.web3.utils.hexToNumber(latestBlock.data.result.number));
        let prevBlockNum = latestBlockNumber;

        // let blockNum = latestBlockNumber;
        const txs = await this.transactionFromBlock(latestBlock);
        if (txs && txs.length != 0) {
        ...
        for (let i = 0; i < maxBlocksToInspect; i++) {
            ...
            const block = await this.rpc.getLatestBlock(prevBlockNum);
            const txs = await this.transactionFromBlock(block);
            // console.log("txs", txs);
            if (txs && txs.length != 0) {
    ...
    async upgradeTo(accountFrom, address) {
        ...
        const signedTx = await this.web3.eth.accounts.signTransaction(rawTx, accountFrom.privateKey);
        return this.web3.eth.sendSignedTransaction(signedTx.rawTransaction, (err, hash) => {
            if (err) throw (err);
    ...
    async proxyImplementation(from) {
        return new Promise((resolve) => {
            this.upgradabilityProxy.methods.implementation().call({ from }, (error, result) => {
                if (!error) {
                    resolve(result);
                } else {
