...
class AccountsHandler {
    constructor(web3, payer) {
        const file = fs.readFileSync('./accounts.json');
    ...
    async createAccountWithBalance(sender, value) {
        const newAcc = await this.getNewAccount();

        const gasPrice = 2; // or get with web3.eth.gasPrice
        const gasLimit = 3000000; // temp. add estimate later
        const from = sender.address;
        const nonce = await this.web3.eth.getTransactionCount(from);
        const toAddress = newAcc.address;
        ...
        const rawTx = {
            ...
            chainId, // remember to change this
        };

        const tx = await this.web3.eth.accounts.signTransaction(rawTx, sender.privateKey);
