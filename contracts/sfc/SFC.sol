 ...
 */
contract SFC is NodeInterface, Validators, Ownable, StakersConstants, Version {
    ...
    */
    ...
    function _setGenesisValidator(address auth, uint256 validatorID, bytes calldata pubkey, uint256 status, uint256 createdEpoch, uint256 createdTime, uint256 deactivatedEpoch, uint256 deactivatedTime) external {
    ...
    function _setGenesisDelegation(address delegator, uint256 toValidatorID, uint256 amount, uint256 rewards) external {
        _rawStake(delegator, toValidatorID, amount);
        rewardsStash[delegator][toValidatorID] = rewards;
    }

    constructor(uint256 _sealedEpoch) public {
        currentSealedEpoch = _sealedEpoch;
    }
