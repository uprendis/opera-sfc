async function callConstant(accounts, methodInstance, isNumber) {
    ...
    for (let i = 0; i < accounts.length; i++) {
        ...
        if (methodInstance == web3.eth.getBalance) { currentValue = await methodInstance(accounts[i]); } else { currentValue = await methodInstance.call(accounts[i]); }
...
function assert_(currentValue, checkValue, isNumber, diff) {
    for (let i = 0; i < currentValue.length; i++) {
        if (isNumber) { assert.equal(currentValue[i] - checkValue[i], diff); } else { assert.equal(currentValue[i], checkValue[i]); }
    }
}
...
module.exports = {
    callConstant, callMethod, assert_, isContract,
};
